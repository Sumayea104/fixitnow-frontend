      ...
      <CardHeader className="p-4 pb-0">
        <div className="flex items-start gap-4">
          <Avatar className="h-14 w-14">
            <AvatarImage src={technician.user?.profileImage ?? undefined} />
            <AvatarFallback>
      ...
      <CardContent className="p-4">
        ...
        <div className="mt-3 flex flex-wrap items-center gap-4 text-sm">
          <div className="flex items-center gap-1">
            ...
            <span className="font-medium">{technician.averageRating?.toFixed(1) ?? 'New'}</span>
            <span className="text-muted-foreground">({technician.totalReviews ?? 0})</span>
          </div>

          {technician.location && (
            <div className="flex items-center gap-1 text-muted-foreground">
              <MapPin className="h-4 w-4" />
              <span className="truncate max-w-30">{technician.location}</span>
            </div>
          )}
